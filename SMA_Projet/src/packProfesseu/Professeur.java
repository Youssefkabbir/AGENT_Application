...
public class Professeur extends GuiAgent {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	//oncréer l'objet gui de l'interface ProfesseurGui
	private ProfesseurGui gui;

	@Override
	protected void setup() {
		//on instancie l' objet ddans la classe ProfesseurGui
		gui=new  ProfesseurGui();
		//
			gui.setCAgent(this);
			//on donne le nom à notre interface
			gui.setTitle(this.getLocalName());

					//System.out.println("Démarage de l' agent Professeur");
...
//la réception de message
	...
	protected void onGuiEvent(GuiEvent ev) {
		...
		//on va créer une Map  pour récupérer les paramétres
			...
			//la récupération de contenu du message saisi dans l'interface
			String contenu=(String)params.get("contenu");
			//la récupération de nom d'agent saisi dans l'interface
			String agentProfesseur=(String)params.get("agentProfesseur");
			//la création du message qu' on va envoyer  à l' agent 
			ACLMessage msg1=new ACLMessage(ACLMessage.REQUEST);
			//on ajoute le nom d' agent récepteur
			msg1.addReceiver(new AID(agentProfesseur,AID.ISLOCALNAME));
			//le contenu de message
			msg1.setContent(contenu);
			//on peut préciser l'ontology
			msg1.setOntology("communication");
			//on evoie le message
			send(msg1);
			break;
		default	:
